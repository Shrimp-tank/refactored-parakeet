...
1. **Build the app once:** In Finder, open the repository folder and double-click `packaging/Build Mac App.command`. macOS may block the script the first time because it was created locally (see "If macOS blocks the build script" below). If your Mac has never had Python&nbsp;3 installed, macOS will also offer to install it—choose **Install** and rerun the script once the installer finishes. After it runs, a short Terminal window appears while the bundle is assembled. When it finishes you will see `Serato Rekordbox Sync.app` inside the new `dist` folder.
...
The exported app already bundles its own Python interpreter, so macOS will not prompt you to install Python when you launch it. If you rebuild the project in the future, just double-click the `.command` file again to refresh the app.
...
- **The app asks for Python:** Rebuild with the latest `Build Mac App.command`. The exported `.app` now includes its own Python interpreter, so launching the new build will not trigger the installer prompt.
